<script lang="ts">
    ...
    onMount(async () => {
        ...
        if (resCourse.ok) {
            ...
            learningOutcomes = data.learning_outcomes.map((lo: any) => ({
                ...
                program_outcomes: lo.program_outcomes.map((po: any) => ({
                    code: po.code,
                    weight: po.weight,
                })),
            }));
            ...
            assessments = data.assessments.map((a: any) => ({
                ...
                learning_outcomes: a.learning_outcomes.map((lo: any) => ({
                    loIndex: learningOutcomes.findIndex(
                        (x) =>
                            `${courseCode}-LO${learningOutcomes.indexOf(x) + 1}` ===
                            lo.learning_outcome,
                    ),
                    weight: lo.weight,
                })),
    ...
    async function updateCourse() {
        ...
        const res = await fetch(
            `http://localhost:8080/api/courses/${courseId}/edit/`,
            {
                method: 'PUT',
                headers: {
...
<div class="min-h-screen bg-[#0b0b10]">
    <div class="max-w-6xl mx-auto px-8 py-10 space-y-12">
        <div class="space-y-1">
            <h1 class="text-3xl font-semibold text-white">Edit Course</h1>
            <p class="text-sm text-gray-400">
                Update learning outcomes and assessments
            </p>
        </div>
